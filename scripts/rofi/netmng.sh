...
function rofi_menu() {
	if [[ $(nmcli dev status | grep -ow ^wlan. | wc -l) -ne "1" ]]; then
		((LINES+=1))
		SELECTION=$(echo -e "$WIFI_LIST\n~Scan\n~Manual\n$WIFI_SWITCH\n$WIRE_SWITCH\n~Change Wifi Interface\n~Status\n~Restart Network" | uniq -u | rofi -dmenu -p "${WIRELESS_INTERFACES[WLAN_INT]} SSID" -lines "$LINES" -a "0" -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -width -"$WIDTH" )
	else
		SELECTION=$(echo -e "$WIFI_LIST\n~Scan\n~Manual\n$WIFI_SWITCH\n$WIRE_SWITCH\n~Status\n~Restart Network" | uniq -u | rofi -dmenu -p "${WIRELESS_INTERFACES[WLAN_INT]} SSID" -lines "$LINES" -a "0" -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -width -"$WIDTH" )
	fi
...
function ssid_manual() {
	...
	SSID=$(echo "Enter SSID" | rofi -dmenu  -p ">_" -mesg -a "0" -lines 1 -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -width 18 )
	echo $SSID
	if [[ ! -z $SSID ]]; then
		PASS=$(echo "Enter Password" | rofi -dmenu  -p ">_" -a "0"  -password -lines 1 -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -width 18   )
		if [ "$PASS" = "" ]; then
				check_wifi_connected
			...
			if [[ ! -z "$SELECTION" ]] && [[ "$WIFI_LIST" =~ .*"$SELECTION".*  ]]; then
				...
				if [[ "$ACTIVE_SSID" == "$SSID_SELECTION" ]]; then
					...
					if [[ "$SELECTION" =~ "WPA2" ]] || [[ "$SELECTION" =~ "WEP" ]]; then
						PASS=$(echo "$PASSWORD_ENTER" | rofi -dmenu -p ">_" -password -a "0" -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -width 16 -lines 1 )
					fi
					if [[ ! -z "$PASS" ]] ; then
...
function status() {
	...
	echo -e "$ETH_STATUS\n${WLAN_STATUS[@]}\n"| rofi -dmenu -p "Status" -lines "$STATUS_LINES" -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -width -"$WIDTH"
}
function main() {
	initialization
	rofi_menu
